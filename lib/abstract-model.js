...
class AbstractModel {
			...
			if (isCustom) { // Custom object
				...
				if (isArray) {
					...
					for (let subValue of attrValue) {
						...
						this._errors[attrName] = validationUtil.validate(attrObject, subValue, validator);
					}
				} else if (typeof CustomModel == typeof attrValue) {
					this[attrName] = attrValue;

					this._errors[attrName] = validationUtil.validate(attrObject, attrValue, validator);
	...
	validate(validatorMode) {
		...
		Object.keys(values).forEach(function(key) {
			self._errors[key] = validationUtil.validate(self._attributes[key], values[key], mode);
		});
...
module.exports = {
	init: function(options) {
		...
		validator = options.validator || 'none';
