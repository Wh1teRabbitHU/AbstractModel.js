...
function isModuleExists(path) {
	if (modelUtil.isInstanceRegistered(path)) {
		return true;
	}
...
function loadCustomModel(_class, modelRoot) {
	...
	if (modelUtil.isInstanceRegistered(_class)) {
		return modelUtil.getInstance(_class);
	} else if (modelUtil.isInstanceRegistered(modelRoot + '/' + _class)) {
		return modelUtil.getInstance(modelRoot + '/' + _class);
	} else if (isModuleExists(modelRoot + '/' + toLowerCammelCase(_class))) { // Relative path
