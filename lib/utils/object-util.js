...
function isValidAttributes(attributes, modelRoot) {
	var attrValidationObject = { invalidAttributes: [] };

	attrValidationObject.isValid = typeof attributes != 'undefined' && attributes !== null;
	...
	Object.keys(attributes).forEach(function(key) {
		...
		if (isCustom(attrType) && !(modelUtil.isInstanceRegistered(attrType) || isModuleExists(modelRoot + '/' + toLowerCammelCase(attrType)))) {
			attrValidationObject.isValid = false;
			attrValidationObject.invalidAttributes.push(attrType);
		}
	});

	return attrValidationObject;
}
...
function loadCustomModel(alias, modelRoot) {
	var path = '';

	if (modelUtil.isInstanceRegistered(alias)) {
		return modelUtil.getInstance(alias);
	} else if (modelUtil.isInstanceRegistered(modelRoot + '/' + alias)) {
		return modelUtil.getInstance(modelRoot + '/' + alias);
	} else if (isModuleExists(modelRoot + '/' + toLowerCammelCase(alias))) { // Relative path
		path = modelRoot + '/' + toLowerCammelCase(alias);
	} else if (isModuleExists(alias)) { // Full path
		path = alias;
	} else {
